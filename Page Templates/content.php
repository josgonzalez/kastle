...
// Posts template page
/* This is the default template from wordpress, only lines 69-96 were changed to add date and classes to 
the date and header and to change the featured image size
*/
?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'tp-panel' ); ?>>
	...
	<header class="entry-header <?php if ( get_theme_mod( 'show_post_banner' ) ) : echo 'post-header'; endif; ?>">
		...
		if ( is_singular() ) :
			...
			the_date('', '<p class="post-date">', '</p>', TRUE);
		else :
